...
try:
    from payment import create_user, gate_analysis, get_user, render_pricing
except Exception as exc:  # pragma: no cover
...
st.set_page_config(
    page_title="BA Assistant — AI-Powered Requirement Analysis",
    page_icon="🚀",
    layout="wide",
...
[data-testid="stAppViewContainer"] {
  background:
    radial-gradient(circle at 0% 0%, rgba(124, 58, 237, 0.12), transparent 30%),
    radial-gradient(circle at 100% 15%, rgba(6, 182, 212, 0.12), transparent 28%),
    radial-gradient(circle at 50% 100%, rgba(34, 197, 94, 0.10), transparent 30%);
}
...
class RequirementAnalyzer:
    def __init__(self, model_id: str = "deepseek-v4-flash", show_member_responses: bool = False, enable_vision: bool = False):
...
def save_history(project_name: str, analysis_type: str, result: str) -> None:
    item = {
    ...
    st.session_state["history"] = [item, *st.session_state.get("history", [])][:10]
...
def sidebar_config() -> Tuple[AppConfig, str, Dict[str, Any]]:
    with st.sidebar:
        st.markdown("### 👤 Account")
        email = st.text_input("Email", value=st.session_state.get("email", ""), placeholder="you@company.com", key="email")
        user: Dict[str, Any] = {}
        if email:
            user = get_user(email) or create_user(email)
            plan = str(user.get("plan", "free")).title()
            used = user.get("usage_count", 0)
            limit = user.get("usage_limit", 2)
            st.caption(f"Plan: **{plan}** · Usage: **{used}/{limit}**")
            render_pricing(email, user)
        else:
            st.info("Enter email to enable pricing, usage limits, and analysis access.")
        ...
        st.markdown("---")
        st.caption("Privacy note: Requirements are sent only to the configured model providers when you run analysis. Do not paste secrets or regulated production data unless your vendor setup permits it.")
        render_footer()
    ...
    return config, email.strip(), user
...
def run_paid_gate(email: str, consume_usage: bool = True) -> bool:
    if not email:
        st.error("Enter your email in the sidebar before running analysis.")
        return False
...
def render_interactive_flow(config: AppConfig, email: str, requirements_text: str) -> None:
    ...
    if stage == "questions":
        ...
        if generate_clicked:
            ...
            save_history(config.project_name, config.analysis_type, result)
...
def main() -> None:
    ...
    config, email, _user = sidebar_config()
    ...
    col_analyze, col_clear, col_diagram = st.columns([2, 1, 1])
    with col_analyze:
        analyze_clicked = st.button(
            "🔍 Analyze" if config.analysis_type != "Interactive (Q&A)" else "Start Interactive Flow Below",
            type="primary",
            use_container_width=True,
            disabled=config.analysis_type == "Interactive (Q&A)",
        )
    with col_clear:
        clear_clicked = st.button("🧹 Clear", use_container_width=True)
    with col_diagram:
        diagram_clicked = st.button("🪄 Diagram", use_container_width=True)
    ...
    with tab_results:
        ...
        if analyze_clicked and config.analysis_type != "Interactive (Q&A)":
            if not requirements_text.strip():
                ...
                placeholder = st.empty()
                with st.spinner("Running BA Assistant agents..."):
                    analyzer = RequirementAnalyzer(config.model_id, config.show_member_responses, enable_vision=False)
                    if config.analysis_type in {"Comprehensive", "Enterprise"}:
                        ...
                        result = stream_to_markdown(
                            lambda stream: analyzer.run_specialized(requirements_text, config.project_name, config.analysis_type, stream=stream),
                            placeholder,
                        )
                st.session_state["last_result"] = result
                st.session_state["last_mermaid"] = extract_mermaid_code(result)
    ...
    with tab_diagrams:
        ...
        mermaid = st.session_state.get("last_mermaid") or extract_mermaid_code(st.session_state.get("last_result", ""))
        if mermaid:
            ...
            st.info("Generate a diagram or run an analysis that includes Mermaid output.")
    ...
    with tab_history:
        ...
        for item in history:
            with st.expander(f"{item['time']} · {item['project']} · {item['type']}"):
